...
- 管理后台相关能力只在客户端预留展示入口或接口兼容，普通用户不展示角色/权限信息，管理员登录后才展示管理入口。
...
- 预留用户上传内容保存到服务端的接口，但首期不启用正文云存储。
- 支持客户端之间分享传输自上传内容，分享时需要携带书籍元数据、内容校验值和必要的本地内容包。
...
- 阅读内容来源包括管理后台统一录入和用户自行上传；用户上传内容处理策略同书籍正文，暂时仅保存在客户端本地，预留服务端保存接口和客户端间分享传输能力。
...
- 展示听力材料列表，支持按难度、时长筛选。
- 听力内容来源包括管理后台统一录入和用户自行上传；用户上传内容处理策略同书籍正文。
...
- 排行榜榜单由后台存储和计算，支持全站总榜、词书榜、班级榜、周榜、月榜等范围。
- 客户端支持用户自建学习小组或组成同桌关系，用于朋友之间共同学习、进度对比和互相激励。
- 排行榜数据可来自 `leaderboard_snapshot`，实时总积分仍可参考 `users.total_score`。
...
| 自上传内容分享包 | `book`、`media_asset`、`content_hash` | 可由本地内容表导出生成 |
...
- 用户自行上传内容暂时仅保存在客户端，预留保存到服务端的接口；客户端之间需要支持分享传输。
- 阅读、听力、口语内容采用“后台统一录入 + 用户自行上传”双来源；用户上传内容处理方式同书籍。
- 排行榜数据在后台存储，范围包括全站总榜、词书榜、班级榜、周榜、月榜等；客户端支持自建学习小组或同桌关系。
- 角色/权限信息仅管理员可见，普通客户端不展示。
