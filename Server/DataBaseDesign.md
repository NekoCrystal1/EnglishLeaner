...
- **书籍正文可外置**：用户上传书籍时，服务端只保存 `book` 元数据、`client_content_ref`、摘要和校验值，正文留在客户端或对象存储。
...
- 用户上传书籍：服务端只保存 `book` 元数据、`client_content_ref`、`content_hash`；正文留在客户端本地 SQLite。后续需要云同步正文时，再将 `content_storage_mode` 从 `CLIENT_ONLY` 切换为 `OSS`。
...
| 表名 | 说明 | 关键字段 |
| --- | --- | --- |
| `reading_passage` | 阅读文章 | `book_id`、`title`、`content`、`summary`、`language`、`level`、`word_count`、`estimated_minutes`、`status` |
...
| 表名 | 说明 | 关键字段 |
| --- | --- | --- |
| `listening_material` | 听力材料 | `book_id`、`title`、`transcript`、`media_asset_id`、`level`、`duration_ms`、`status` |
...
| `speaking_task` | 口语任务 | `title`、`prompt_text`、`reference_text`、`media_asset_id`、`level`、`score_config_json`、`status` |
| `speaking_attempt` | 口语作答/评测记录 | `user_id`、`task_id`、`audio_asset_id`、`recognized_text`、`score_total`、`score_detail_json`、`duration_ms`、`attempted_at`、`client_event_id` |

第三方语音评测 SDK 后续接入时，只需要把评测结果结构化存入 `speaking_attempt.score_detail_json`，必要指标再提升为结构化字段。
...
| `leaderboard_snapshot` | 排行榜快照，可选 | `scope_type`、`scope_id`、`rank_date`、`user_id`、`rank_no`、`score` |

说明：

- 实时排行榜可直接按 `users.total_score` 查询。
- 大用户量或按周/月排行时，写入 `leaderboard_snapshot`，避免每次全表排序。
...
| `V7__extended_learning_modules.sql` | 创建阅读、听力、口语相关表 |
| `V8__gamification_stats_sync.sql` | 创建打卡、成就、积分流水、日汇总、同步明细表 |
...
- 当前排行榜仍可读 `users.total_score`，后续由 `score_transaction` 汇总维护。
...
- 用户上传书籍是否永远只存在客户端，还是后续允许同步到对象存储。
- 阅读、听力、口语内容是否需要管理后台录入，还是只作为客户端本地学习记录。
- 排行榜范围是否只有全站总榜，还是需要按词书、班级、周榜/月榜扩展。
- 管理端是否需要细粒度权限，还是先保留 `ADMIN` / `USER` 两级角色。
- 语音评测接入 SDK 后，音频文件是否上传服务端对象存储。
